...
exports.saveAppForm = async (req, res) => {
    ...
    const ApplyWithAction = addLastAction(Apply, "saveAppForm", common_1.getCurrentDate("Y-m-d H:i:s"));
    const ApplyRepository = getRepository(ApplyEntity);
    const ApplyConfirmRepository = getRepository(ApplyConfirmEntity);
    ...
    try {
        ...
        if (Files) {
            ...
            for (const [key, value] of Object.entries(Files)) {
                const Ext = common_1.getExtension(value.name);
                const FilePath = "/upload/apply/";
                const FileName = "APPLY" + common_1.getCurrentDate("YmdHis") + Index + "." + Ext;
                Index++;
                console.log(Ext);
...
exports.saveApply = async (req, res) => {
    ...
    try {
        ...
        const Result = await ApplyRepository
            ...
            .set({
            APPLY_STATUS: 2,
            APPLY_COMPLETE_DATE: common_1.getCurrentDate("Y-m-d H:i:s")
        })
            .where("APPLY_SEQ = :APPLY_SEQ", { APPLY_SEQ: ApplySeq })
            .execute().catch((err) => {
            console.log(err);
...
exports.applyWork = async (req, res) => {
    ...
    try {
        ...
        UpdateData.APPLY_ACTION_DATE = common_1.getCurrentDate("Y-m-d H:i:s");
    ...
    try {
        await createUserAction(UserSeq, "TBL_FW_APPLY", "editApplyAtWork", "SUCCESS")
            ...
            ApplyRepository
                ...
                APPLY_ACTION_DATE: common_1.getCurrentDate("Y-m-d H:i:s")
            })
                .where("APPLY_SEQ = :APPLY_SEQ", { APPLY_SEQ: ApplySeq })
                .execute().catch((err) => {
                console.log(err);
...
exports.confirmApprove = async (req, res) => {
    ...
    try {
        ...
        const Result = await ApplyConfirmRepository
            ...
            CONFIRM_DATE: common_1.getCurrentDate("Y-m-d H:i:s")
        })
...
async function createUserAction(userSeq, tableName, functionName, result) {
    ...
    const UserAction = {
        ...
        USER_ACTION_DATE: common_1.getCurrentDate("Y-m-d H:i:s"),
    };
...
//# sourceMappingURL=applyModel.js.map